<section class="flex h-screen w-screen items-stretch overflow-hidden bg-[#fafafa] text-zinc-900">
    ...
    <aside class="flex h-full w-[410px] shrink-0 flex-col overflow-y-auto px-10 py-9">
        ...
        {{-- Lower sidebar breathing room --}}
        <div class="mt-auto"></div>
    </aside>

    {{-- Main panel --}}
    <div class="flex h-full min-w-0 flex-1 flex-col border-l border-zinc-200/70 bg-white">
        <div class="flex-1 overflow-y-auto px-10 pt-[55px] lg:px-14 flex items-center justify-center">
            <div class="mx-auto max-w-5xl">
                {{-- Header --}}
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-base tracking-wide text-zinc-500 font-medium">Select a Template</h2>
                    </div>
                </div>
                ...
                <div class="mt-4 grid grid-cols-1 gap-5 xl:grid-cols-2">
                    @foreach ($templates as $template)
                        <label wire:key="template-{{ $template['key'] }}" class="group relative flex cursor-pointer gap-5 rounded-xl border border-zinc-200 bg-white p-3 transition-colors hover:border-zinc-300 has-[:checked]:border-zinc-900 has-[:checked]:ring-1 has-[:checked]:ring-zinc-900">
                            <input type="radio" name="template" value="{{ $template['key'] }}" class="peer sr-only" @checked($template['key'] === $defaultTemplate)>

                            {{-- Selection control --}}
                            <span aria-hidden="true" class="pointer-events-none absolute right-4 top-4 h-6 w-6 rounded-full border-2 border-zinc-300 bg-white transition-colors peer-checked:border-zinc-900 peer-checked:bg-zinc-900"></span>
                            <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="pointer-events-none absolute right-4 top-4 h-6 w-6 p-1 text-white opacity-0 transition-opacity peer-checked:opacity-100"><path d="M20 6 9 17l-5-5"/></svg>

                            {{-- Thumbnail placeholder --}}
                            <div class="hidden h-32 w-44 shrink-0 overflow-hidden rounded-lg border border-zinc-100 bg-zinc-50 sm:block">
                                <div class="flex h-full flex-col">
                                    <div class="flex items-center gap-1 border-b border-zinc-100 px-2.5 py-2">
                                        <span class="h-1.5 w-1.5 rounded-full bg-zinc-200"></span>
                                        <span class="h-1.5 w-1.5 rounded-full bg-zinc-200"></span>
                                        <span class="h-1.5 w-1.5 rounded-full bg-zinc-200"></span>
                                    </div>
                                    <div class="flex-1 space-y-2 p-3">
                                        <div class="h-1.5 w-3/4 rounded-full bg-zinc-200/80"></div>
                                        <div class="h-1.5 w-1/2 rounded-full bg-zinc-200/70"></div>
                                        <div class="h-1.5 w-2/3 rounded-full bg-zinc-200/60"></div>
                                        <div class="h-1.5 w-2/5 rounded-full bg-zinc-200/50"></div>
                                    </div>
                                </div>
                            </div>

                            {{-- Content --}}
                            <div class="min-w-0 flex-1 py-2 pr-8">
                                <h3 class="text-lg font-semibold text-zinc-900">{{ $template['name'] }}</h3>
                                <p class="mt-1.5 text-sm leading-relaxed text-zinc-500">{{ $template['description'] }}</p>
                            </div>
                        </label>
                    @endforeach
                </div>
        ...
        {{-- Footer --}}
        <div class="flex shrink-0 items-center justify-between border-t border-zinc-200/70 bg-white px-10 py-5 lg:px-14">
            <button type="button" class="inline-flex items-center gap-2 rounded-xl border border-zinc-200 bg-white px-5 py-3 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-50">
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
                Back
            </button>
            <button type="button" class="inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-7 py-3 text-sm font-semibold text-white transition-colors hover:bg-zinc-800">
                Continue
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </button>
        </div>
    </div>
</section>
